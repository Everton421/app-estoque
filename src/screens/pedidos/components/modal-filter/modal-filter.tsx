...
import { ModalFilterStatusSeparation } from "../modal-filter-status-separation/modal-filter-status-separation";
...
export const ModalFilter = ({setFilter, filter,  visible, setVisible,    }: ModalFilterProps) => {
    ...
    const [ isVisibleModalStatusSeparation, setIsVisibleModalStatusSeparation] = useState(false);
    const [ statusSeparationOrder, setStatusSeparationOrder] = useState<statusSeparation[]>([])
    ...
    const handleSelectSituationSeparation = (situationSeparation: statusSeparation)=>{
        setFilter({ type: 'switch_status_separation',payload: situationSeparation })
    }
    ...
    return (
              ...
              >
                ...
                <TouchableOpacity style={{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0 }} activeOpacity={1} onPress={() => setVisible(false)} />
                    <View style={{ width: '100%',height:'90%',marginTop: 50,  backgroundColor: "#FFF",borderTopRightRadius:16,borderTopLeftRadius:16,elevation: 10,overflow: 'hidden' }}>
                        ...
                        <TouchableOpacity
                            onPress={() => setIsVisibleModalStatusSeparation(true)}
                            style={{flexDirection: 'row',alignItems: 'center',justifyContent: 'space-between',backgroundColor: '#F5F7FA',padding: 12,borderRadius: 8,borderWidth: 1,borderColor: '#E0E0E0',marginTop: 10  }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10 }}>
                            <MaterialCommunityIcons name='package-variant-plus' size={24} color="#185FED" />
                                <Text style={{ fontSize: 16, color: '#333', fontWeight: '500' }}>
                                    Status Separação 
                                </Text>
                            </View>
                            <Ionicons name={isVisibleModalStatusSeparation? "chevron-up" : "chevron-down"} size={20} color="#666" />
                        </TouchableOpacity>
                    ...
                    <ModalFilterStatusSeparation
                        selectedFilter={ filter.status_separacao }
                        setSelectedFilter={handleSelectSituationSeparation}
                        setVisible={setIsVisibleModalStatusSeparation}
                        visible={isVisibleModalStatusSeparation}
                    />
                        </View>
                </View>
            </View>
