...
type propsSeller = {
  ...
  const RenderSeller = ({ setVisible, seller, selectSeller, sellerSelected }: propsSeller) => {
    return (
        <TouchableOpacity
            style={{
                backgroundColor:   seller.codigo == sellerSelected ? '#f4fff2' : '#FFF' ,
                marginHorizontal: 10,
                marginVertical: 5,
                borderRadius: 12,
                padding: 15,
        ...
        >
            <View style={{
                ...
                <MaterialIcons name="people-alt" size={24} color={ seller.codigo == sellerSelected ? '#1ea902f0' : '#185FED'} />
            </View>
            ...
            <View style={{ flex: 1 }}>
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4 }}>
                    <Text style={{ fontSize: 12, color:  seller.codigo == sellerSelected ? '#1ea902f0' : '#185FED', fontWeight: 'bold', backgroundColor: '#E3F2FD', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 4, marginRight: 8 }}>
                        Cód: {seller.codigo}
                    </Text>
...
export const ModalSeller = ({ visible, setVisible,  selectSeller,  sellerSelected }: props)=>{
    const api = useApi();
        ...
        <Modal visible={visible} transparent={true} animationType="fade" onRequestClose={() => setVisible(false)}>
                <View style={{ flex: 1, backgroundColor: "rgba(0, 0, 0, 0.5)", justifyContent: 'center', alignItems: 'center' }}>
                    <View style={{ width: '90%', height: '80%', backgroundColor: "#FFF", borderRadius: 16, overflow: 'hidden', elevation: 10 }}>
                        ...
                        <FlatList
                            data={dataBranches}
                            renderItem={({ item }) => <RenderSeller setVisible={setVisible} selectSeller={selectSeller}  seller={item} sellerSelected={sellerSelected}/>}
