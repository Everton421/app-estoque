...
export const Cadastro_Setores = ({ navigation }: any) => {
    ...
    async function findLastCode() {
        const resultRequestlastCode = await api.get("/setores/last-codigo");
    ...
    async function gravar() {
        ...
        if (!input || input === "") {
            setIsVisibleAlert(true);
            setTitleAlert("Atenção!");
            setMessageAlert("É necessario informar a descrição do setor");
            setTypeAlert('warning');
            setCancelText(undefined);
            setConfirmText('ok');
            return;
        ...
        try {
            ...
            let resposta = await api.post('/setores', { "descricao": input, id: newId});

            if (resposta.status === 201 && resposta.data.codigo > 0) {

                setIsVisibleAlert(true);
                setTitleAlert("Sucesso");
                setMessageAlert(`Setor ${input} registrado com sucesso! `);
        ...
        <View style={{ flex: 1, backgroundColor: '#F0F4F8' }}>
            ...
            <ScrollView contentContainerStyle={{ padding: 16 }}>
                <View style={{ backgroundColor: '#FFF', borderRadius: 12, padding: 16, elevation: 3, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.1, shadowRadius: 4 }}>
                    ...
                    <Text style={{ fontWeight: '600', fontSize: 14, color: '#6C757D', marginBottom: 6 }}>Descrição do Setor</Text>
                    <TextInput
                ...
                <TouchableOpacity
                    ...
                    onPress={() => gravar()}
                >
                    <Text style={{ fontWeight: 'bold', color: '#FFF', fontSize: 18 }}>Gravar Setor</Text>
                </TouchableOpacity>
