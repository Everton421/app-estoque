...
export const receberPedidos = ()=>{
  ...
  const api = useApi();
    ...
    async function getPedidos({ data ,setIsLoading, setProgress,   setItem } : propsSyncData) {
           ...
           try{
               ...
               if( orcamentosSistema.data.length > 0 ){
                ...
                let aux = useQuerypedidos.newUpdate({ contato:'0'}, 1 );
                   ...
                   for (let index = 0; index < orcamentosSistema.data.length; index++) {
                       const i = orcamentosSistema.data[index];
           ...
           try{
               ...
               if( orcamentosSistema.data.length > 0 ){
                ...
                let aux = useQuerypedidos.newUpdate({ contato:'0'}, 1 );
                   ...
                   for (const i of orcamentosSistema.data) {
